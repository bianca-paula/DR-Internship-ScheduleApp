...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['scheduled_id'])) {
    ...
    // Get user schedule
    $query = getScheduledForUserQuery($_GET['user_id']);
    $statement = $db->connection->query($query);
    $schedule_array = $statement->fetchAll(PDO::FETCH_OBJ);
    
    
      
    // Add alternatives to the table
    foreach($alternatives_array as $alternative){
        if($alternative->from_date != $scheduled_course -> from_date && checkTimePeriodIsFree($schedule_array, $alternative->from_date)){  
        // Do not include the course already on the schedule
            ...
            printf("<tr> <td ondblclick = changeCourse('%s') }> %s </td> </tr>",
                $name . "','" . $type . "','" . $weekday . "','" . $start_hour . "','" . $end_hour, // changeCourse parameters
